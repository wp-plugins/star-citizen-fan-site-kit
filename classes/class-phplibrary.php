...
//class STARCITIZENFANSITEKIT_PHP extends STARCITIZENFANSITEKIT_Flags { commented 14092014
class STARCITIZENFANSITEKIT_PHP {  
    /**
    ...
    */
    public function arrayinfo_set( $array, $line, $function, $file ){
        ...
        $array['arrayinfo']['version'] = $starcitizenfansitekit_currentversion;
        $array['arrayinfo']['line'] = $line;
        $array['arrayinfo']['function'] = $function;
        $array['arrayinfo']['file'] = $file;
        $array['arrayinfo']['date'] = STARCITIZENFANSITEKIT::datewp();
        return $array;   
    }  
    ...
    * 2. Faster than starcitizenfansitekit_count_extension_in_directory() because it returns sooner, important for directory with a lot of files
    * 
    * @returns boolean
    */
    ...
    */
    public function type( $value ){
        if( STARCITIZENFANSITEKIT::is_image_url( $value ) ){
            return 'image';
        }
        
        // so its not an image url, is it any type of url? either for using as link or PHP functionality
        if( STARCITIZENFANSITEKIT::is_url( $value ) ){
            return 'url';    
        }
        
        // check if decimal before checking if a plain number or money
        if( STARCITIZENFANSITEKIT::is_decimalnumber( $value ) ){
            return 'decimal';
        }

        // is money value
        if( STARCITIZENFANSITEKIT::is_string_money( $value ) ){
            return 'money'; 
        }
        ...
        // is email address
        if( STARCITIZENFANSITEKIT::is_valid_emailaddress( $value ) ){
            return 'email';
        }                  
    ...
    */
    public function months_time_range( $month, $year, $return = 'array', $format = 'time' ){
        ...
        if( $format == 'date' )
        {
            $array['start'] = STARCITIZENFANSITEKIT::datewp(0, $array['start'] );
        }
        ...
        if( $format == 'date' )
        {    
            $array['end'] = STARCITIZENFANSITEKIT::datewp(0, $array['end'] );   
        }
...
}// end class STARCITIZENFANSITEKIT_PHP
?>
