...
* @version 1.0
*/
class STARCITIZENFANSITEKIT_Formbuilder extends STARCITIZENFANSITEKIT_UI {
    ...
    public function __construct() {
        ...
        $this->WPCore = STARCITIZENFANSITEKIT::load_class( 'STARCITIZENFANSITEKIT_WPCore', 'class-wpcore.php', 'classes' );
        
        // get form validation array
        $this->form_val_arr = get_option( 'starcitizenfansitekit_formvalidation' );
        
        // re-populate class var with the localized value
        $this->first_switch_label = __( 'Enabled', 'starcitizenfansitekit' );
        $this->second_switch_label = __( 'Disabled', 'starcitizenfansitekit' );
    }
    ...
    */
    public function input( $formid, $inputtype, $inputid, $inputname, $optiontitle, $titleatt = '', $required = false, $currentvalue = null, $atts_array = array(), $validation_array = array() ) {
        ...
        if( !in_array( $inputtype, $this->input_types, false ) ) {
            self::option_subline( __( 'This is a technical matter please report it. There should be a form input here that you may require before you continue.', 'starcitizenfansitekit' ), __( 'Invalid Input Type', 'starcitizenfansitekit' ) );
            return false;    
        }      
    ...
    */
    public function register_form( $form_id ) {
        ...
        update_option( 'starcitizenfansitekit_formvalidation', $this->form_val_arr );
    }
    ...
    */
    public function deregister_form( $form_id ) {
        ...
        update_option( 'starcitizenfansitekit_formvalidation', array() );
    }
        ...
        foreach( $this->form_val_arr[ $user_ID ][ $formid ] as $input_name => $input_array ) { 
            ...
            if( isset( $input_array['required']) && $input_array['required'] === true ) {
                ...
                if( $input_array['inputtype'] === 'checkboxes' ) {
                    ...
                    if( $box_checked === 0 ) {
                        $this->UI->create_notice( sprintf( __( 'Oops! You missed something. Please make selection for %s as it is a required option.', 'starcitizenfansitekit' ), $input_array['optiontitle'] ), 'error', 'Small', __( 'Required Option', 'starcitizenfansitekit' ) );                    
                        return false;                    
                    }                    
                    
                    // if not enough checked FAIL
                     if( $box_checked < $input_array['minimumchecks'] ) {
                        $this->UI->create_notice( sprintf( __( 'You have not checked enough boxes for the %s option.', 'starcitizenfansitekit' ), $input_array['optiontitle'] ), 'error', 'Small', __( 'Check More Boxes', 'starcitizenfansitekit' ) );                    
                        return false;                    
                    }
                                       
                    // if too many checked....you guessed it FAIL
                    if( $box_checked > $input_array['maximumchecks'] ) {
                        $this->UI->create_notice( sprintf( __( 'You have checked too many boxes for the %s option.', 'starcitizenfansitekit' ), $input_array['optiontitle'] ), 'error', 'Small', __( 'Please Uncheck Boxes', 'starcitizenfansitekit' ) );                    
                        return false;                    
                    }                    
                    ...
                    $this->UI->create_notice( sprintf( __( 'Form incomplete because the %s is a required option.', 'starcitizenfansitekit' ), $input_array['optiontitle'] ), 'error', 'Small', __( 'Required Option', 'starcitizenfansitekit' ) );                    
                    return false;                      
                }
            }           
        }
    ...
    */
    public function apply_input_security() {
        ...
        if( !isset( $_POST['starcitizenfansitekit_form_formid'] ) ) { 
            return false;
        }
        
        $form_id = $_POST['starcitizenfansitekit_form_formid'];
        ...
        foreach( $_POST as $input_name => $value ) {
            ...
            if( isset( $this->form_val_arr[ $user_ID ][ $form_id ][ $input_name ] ) ) {
                ...
                if( isset( $this->form_val_arr[ $user_ID ][ $form_id ][ $input_name ]['disabled'] ) && $this->form_val_arr[ $user_ID ][ $form_id ][ $input_name ]['disabled'] === true ) {
                    ...
                    $this->UI->create_notice( __( 'Possible security violation detected. Please discontinue your action. This event has been logged for investigation.', 'starcitizenfansitekit' ), 'warning', 'Small', __( 'Warning', 'starcitizenfansitekit' ) );                    
                    return false;    
                }
            }    
        }                  
            ...
            foreach( $this->form_val_arr[ $user_ID ][ $form_id ] as $input_name => $input_array ) {
                ...
                if( !in_array( $this->form_val_arr[ $user_ID ][ $form_id ][ $input_name ]['inputtype'], $this->grouped_fields ) ) {
                    ...
                    if( isset( $this->form_val_arr[ $user_ID ][ $form_id ][ $input_name ]['disabled'] ) && $this->form_val_arr[ $user_ID ][ $form_id ][ $input_name ]['disabled'] === false ) {
                        ...
                        if( !isset( $_POST[ $input_name ] ) ) {   
                            $this->UI->create_notice( __( 'Possible security violation. Please discontinue your action. This event has been logged for investigation.', 'starcitizenfansitekit' ), 'warning', 'Small', __( 'Warning', 'starcitizenfansitekit' ) );                    
                            return false;                        
                        } 
                    }
                }
                ...
                if( $input_array['inputtype'] == 'menu' ) {
                    ...
                    if( $found_match === false ) {
                        $this->UI->create_notice( __( 'Star Citizen Fan Site Kit has security related concerns about a menu on the form you submitted. Please discontinue your action and contact support. This event has been logged for investigation.', 'starcitizenfansitekit' ), 'warning', 'Small', __( 'Warning', 'starcitizenfansitekit' ) );                    
                        return false;    
                    }
                }                
                ...
                if( $input_array['inputtype'] == 'checkboxes' ) {
                    ...
                    if( isset( $this->form_val_arr[ $user_ID ][ $form_id ][ $input_name ]['groupcheckboxes'] ) && $this->form_val_arr[ $user_ID ][ $form_id ][ $input_name ]['groupcheckboxes'] == false ) {
                        ...
                        if( $failed_to_match === true ) {
                            $this->UI->create_notice( __( 'WTG CSV Exporter security has detected a possible problem with the form you submitted. Please discontinue your action and contact support.', 'starcitizenfansitekit' ), 'warning', 'Small', __( 'Warning', 'starcitizenfansitekit' ) );                    
                            return false;    
                        }
                        ...
                        if( isset( $_POST[ $input_name ] ) && is_array( $_POST[ $input_name ] ) ) {
                            ...
                            // if one or more seletions made but no match this indicates values have been tampered with
                            if( $failed_to_match === true ) {
                                $this->UI->create_notice( __( 'WTG CSV Exporter detected problem relating to the security of the form you submitted. Please discuss this on the WebTechGlobal Forum.', 'starcitizenfansitekit' ), 'error', 'Small', __( 'Something Went Wrong', 'starcitizenfansitekit' ) );                    
                                return false;    
                            }
                        }
                    }    
                ...
                if( $input_array['inputtype'] == 'radiogroup' ) {
                    ...
                    // if no match, the submitted item value was not registered when the form was parsed
                    if( $found_match === false ) {
                        $this->UI->create_notice( __( 'Star Citizen Fan Site Kit detected a problem and could not continue. Please try again and if it continues contact support.', 'starcitizenfansitekit' ), 'warning', 'Small', __( 'Warning', 'starcitizenfansitekit' ) );                    
                        return false;    
                    }
                }               
                ...
                if( $input_array['inputtype'] == 'hidden' ) {
                    ...
                    if( !isset( $_POST[ $input_name ] ) ) {
                       // arriving here indicates hidden input missing, a hack has removed it
                        $this->UI->create_notice( __( 'A security matter has arisen. Please discontinue your action and contact support if this message continues to appear. The event has been logged.', 'starcitizenfansitekit' ), 'warning', 'Small', __( 'Warning', 'starcitizenfansitekit' ) );                    
                        return false;                         
                    } else {
                        // if registered value does not match value submitted, hacker has changed it
                        if( $input_array['currentvalue'] !== $_POST[ $input_name ] ) {
                            $this->UI->create_notice( __( 'Star Citizen Fan Site Kit has detected a possible hack attempt. Your activity has been logged. Please discontinue your action and contact support if this message continues to appear without any known reason.', 'starcitizenfansitekit' ), 'warning', 'Small', __( 'Warning', 'starcitizenfansitekit' ) );                    
                            return false;                            
                        }
                    }
                }   
    ...
    */    
    public function user_can( $capability = 'activate_plugins' ){
        if(!current_user_can( $capability ) ){
            $this->UI->create_notice( __( 'Your request was rejected as your account does not have the required permission.', 'starcitizenfansitekit' ), 'success', 'Small', __( 'Not Permitted', 'starcitizenfansitekit' ) );          
            return false;
        }   
        return true;
    }     
    ...
    */
    public function form_start( $form_id, $form_name, $form_title, $uploader = false ){
        global $starcitizenfansitekit_page_name;
        ...
        echo '<input type="hidden" name="starcitizenfansitekit_admin_action" value="true">';
        echo '<input type="hidden" name="starcitizenfansitekit_hidden_pagename" value="' . $starcitizenfansitekit_page_name . '">';
        echo '<input type="hidden" name="starcitizenfansitekit_form_formid" value="' . $form_name . '">';
        echo '<input type="hidden" name="starcitizenfansitekit_form_name" value="' . $form_name . '">';
        echo '<input type="hidden" name="starcitizenfansitekit_form_title" value="' . $form_title . '">';        
    }
    ...
    */
    public function validate_alpha( $value, $optiontitle, $atts_array ) {
        if( !ctype_alpha( $value ) ) {
            $this->UI->create_notice( sprintf( __( "The %s input is only for letters, please edit your entry and remove any numbers or special characters.", 'starcitizenfansitekit' ), $optiontitle ), 'error', 'Small', __( 'Invalid Entry', 'starcitizenfansitekit' ) );               
            return false;            
        }
        return true;
    }
    ...
    */
    public function validate_alphanumeric( $value, $optiontitle ) {
        if( !ctype_alnum( $value ) ) {   
            $this->UI->create_notice( sprintf( __( "The %s input is for letters or numbers, no special characters allowed.", 'starcitizenfansitekit' ), $optiontitle ), 'error', 'Small', __( 'Invalid Entry', 'starcitizenfansitekit' ) );               
            return false;
        }
        return true;
    }
    ...
    */
    public function validate_url( $url, $optiontitle ) {    
        if( !$this->PHP->validate_url( $url ) ) {
            $this->UI->create_notice(sprintf( __( "You did not enter a valid URL in the %s input. Please ensure that all characters match the online URL.", 'starcitizenfansitekit' ), $optiontitle ), 'error', 'Small', __( 'Invalid Entry', 'starcitizenfansitekit' ) );               
            return false;
        }        
        return true;
    } 
    ...
    */
    public function validate_numeric( $value, $optiontitle ) {  
        if( !ctype_digit( $value ) ) {
            $this->UI->create_notice( sprintf( __( "Only numbers are meant to be entered into the %s field.", 'starcitizenfansitekit' ), $optiontitle ), 'error', 'Small', __( 'Invalid Entry', 'starcitizenfansitekit' ) );               
            return false;
        }        
        return true;    
    }
    ...
    */
    public function validate_maxlength( $str, $optiontitle, $atts_mixed ) {
        if ( strlen( $str ) > $atts_mixed ) {
            $this->UI->create_notice( sprintf( __( "The maximum length for the %s field is %s ", 'starcitizenfansitekit' ), $optiontitle, $atts_mixed ), 'error', 'Small', __( 'Too Long', 'starcitizenfansitekit' ) );               
            return false;
        }        
        return true;        
    }    
    ...
    */
    public function validate_minlength( $str, $optiontitle, $atts_mixed ) {
        if ( strlen( $str ) < $atts_mixed ) {
            $this->UI->create_notice( sprintf( __( "The minimum length for the %s field is %s ", 'starcitizenfansitekit' ), $optiontitle, $atts_mixed ), 'error', 'Small', __( 'Too Short', 'starcitizenfansitekit' ) );               
            return false;
        }        
        return true;        
    }
    ...
    */
    public function validate_regex( $str, $optiontitle, $atts_mixed ) {
        if ( !preg_match( $atts_mixed[0], $str ) ) {
            $this->UI->create_notice( sprintf( __( "The %s field entry is not the correct format. A specific pattern of characters is required i.e. %s.", 'starcitizenfansitekit' ), $optiontitle, $atts_mixed[1] ), 'error', 'Small', __( 'Invalid Value', 'starcitizenfansitekit' ) );               
            return false;
        }        
        return true;    
    }
    ...
    */
    public function input_switch(){
        ...
        <!-- Option Start -->
        <tr valign="top">
            <th scope="row"><?php _e( $this->optiontitle, 'starcitizenfansitekit' ); ?></th>
            <td>
        ...
        <tr valign="top">
            ...
            <td>                                     
                <select name="<?php echo $this->inputname;?>" id="<?php echo $this->inputid;?>"<?php if( $this->disabled ){ echo ' disabled';} ?>>
                    ...
                    <option value="notselected" <?php echo $selected;?>><?php _e( 'None Selected', 'starcitizenfansitekit' ); ?></option> 
    ...
    */
    public function input_menu_cronrepeat() {
        ...
        <tr valign="top">
            ...
            <td>            
                <select name="<?php echo $this->inputname;?>" id="<?php echo $this->inputid;?>"<?php if( $this->disabled ){ echo ' disabled';} ?>>
                    <option value="hourly" <?php if( $this->currentvalue == 'hourly' ){ echo $selected; }?>><?php _e( 'Hourly', 'starcitizenfansitekit' ); ?></option>
                    <option value="twicedaily" <?php if( $this->currentvalue == 'twicedaily' ){ echo $selected; }?>><?php _e( 'Twice Daily', 'starcitizenfansitekit' ); ?></option>
                    <option value="daily" <?php if( $this->currentvalue == 'daily' ){ echo $selected; }?>><?php _e( 'Daily', 'starcitizenfansitekit' ); ?></option>
                </select>                  
            </td>
        </tr>
        <!-- Option End --><?php     
    ...
    */
    public function hidden_basic( $form_id, $id, $name, $current_value ) {
        self::input( $form_id, 'hidden', $id, $name, __( 'Hidden Input ' . $id, 'starcitizenfansitekit' ), __( 'Hidden Input ' . $id, 'starcitizenfansitekit' ), true, $current_value, array(), array() ); 
    }
