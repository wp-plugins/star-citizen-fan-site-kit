...
* @author Ryan Bayne 
*/                                                 
//class STARCITIZENFANSITEKIT_Files extends STARCITIZENFANSITEKIT { commented 14092014
class STARCITIZENFANSITEKIT_Files { 
    
    public function __construct() {
        $this->PHP = STARCITIZENFANSITEKIT::load_class( 'STARCITIZENFANSITEKIT_PHP', 'class-phplibrary.php', 'classes' ); # php library by Ryan R. Bayne  
    }
    ...
    */
    public function upload_error( $file_error ) {
        $upload_error_strings = array( false,
            __( "The uploaded file exceeds the <code>upload_max_filesize</code> directive in <code>php.ini</code>.", 'starcitizenfansitekit' ),
            __( "The uploaded file exceeds the <em>MAX_FILE_SIZE</em> directive that was specified in the HTML form.", 'starcitizenfansitekit' ),
            __( "The uploaded file was only partially uploaded.", 'starcitizenfansitekit' ),
            __( "No file was uploaded.", 'starcitizenfansitekit' ),
            __( "Missing a temporary folder.", 'starcitizenfansitekit' ),
            __( "Failed to write file to disk.", 'starcitizenfansitekit' ) );  
    ...
    */
    public function singlefile_uploader( $file_array, $uploads = false, $test_size = true, $test_type = false, $overwrite = false ) {
        $result_array = array( 'outcome' => false,
                               'error' => false, 
                               'message' => __( 'The file was uploaded.', 'starcitizenfansitekit' ) );
        ...
        if ( $test_size && !( $file_array['size'] > 0 ) ) {
            $result_array['outcome'] = false;
            $result_array['error'] = true;
            $result_array['message'] = __( 'Your file is empty. Empty files are not permitted in this operation.', 'starcitizenfansitekit' );
            return $result_array;            
        }
        ...
        if (! @ is_uploaded_file( $file_array['tmp_name'] ) ) {
            $result_array['outcome'] = false;
            $result_array['error'] = true;
            $result_array['message'] = __( 'Sorry your file upload failed. A temporary file could not be created on the server. This may be a temporary problem, please try again.', 'starcitizenfansitekit' );
            return $result_array;                    
        }
        ...
        if ( $test_type ) {
            ...
            if ( ( !$type || !$ext ) && !current_user_can( 'unfiltered_upload' ) ) {
                $result_array['outcome'] = false;
                $result_array['error'] = true;
                $result_array['message'] = __( 'Your file does not meet security requirements and has not been uploaded.', 'starcitizenfansitekit' );
                return $result_array;                
            }
        ...
        if ( false === @ move_uploaded_file( $file_array['tmp_name'], $uploads['path'] . $filename ) ) {
                $result_array['outcome'] = false;
                $result_array['error'] = true;
                $result_array['message'] = __( 'Your file could not be moved into the set directory. If a file with the same name exists in that directory an overwrite may not be possible.', 'starcitizenfansitekit' );
                return $result_array; 
        }    
    ...
    */
    public function file_from_url( $url, $uploads = array(), $overwrite = false, $allow = false ) {
        ...
        if( !$url_is_valid ) {
            $result['message'] = __( 'URL is invalid', 'starcitizenfansitekit' );
            return false;    
        }
        ...
        if ( false !== $uploads['error'] ) {
            $result['message'] = __( 'WordPress wp_upload_dir() returned an error.', 'starcitizenfansitekit' );
        ...
        if( $write_result === false ) {
            ...
            if( file_exists( $uploads['path'] . $file['originalName'] ) ) {
                $result['message'] = __( 'The filename already exists and the file cannot be overwritten.', 'starcitizenfansitekit' );
                return $result;                 
            }
            
        } elseif( $write_result === false && $rename !== true ) {
            $result['failurereason'] = __( 'File already exists and renaming is not allowed.', 'starcitizenfansitekit' );
            return $result;                
        }
        ...
        $message = __( 'The file was transferred.', 'starcitizenfansitekit' ); 
        
        if( $overwrite_applied ) {
            $message .= ' ' . __( 'An existing file was overwritten as requested.', 'starcitizenfansitekit' );
        }
        
        if( $filerename_applied ) {
            $message .= ' ' . __( 'The files name had to be changed as it already exists.', 'starcitizenfansitekit' );    
        }
    ...
    */
    public function compare_csv_files( $filepath_one, $filepath_two ) {
        ...
        can switch to the newer file if required.', 'starcitizenfansitekit' );      
