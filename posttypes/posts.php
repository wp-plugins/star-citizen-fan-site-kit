...
add_action( 'init', 'starcitizenfansitekit_init_posttype_contentdesigns' );
add_action( 'add_meta_boxes', 'starcitizenfansitekit_add_meta_boxes_post' );
add_action( 'save_post', 'starcitizenfansitekit_save_meta_boxes_post',10,2 );
/**
* Must be called using add_action( 'init', 'starcitizenfansitekit_register_customposttype_contentdesigns' )
* Registers custom post type for content only
*/
function starcitizenfansitekit_init_posttype_contentdesigns() {
    $labels = array(
        'name' => _x( 'Star Citizen Fan Site Kit Content Templates', 'post type general name' ),
        'singular_name' => _x( 'Star Citizen Fan Site Kit Content Template', 'post type singular name' ),
        ...
        'parent_item_colon' => '',
        'menu_name' => 'STARCITIZENFANSITEKIT Content'
    );
    $args = array(
        'labels' => $labels,
        'public' => false,
...
*/
function starcitizenfansitekit_add_meta_boxes_post() {
    ...
    //if( $starcitizenfansitekit_settings['monetizesection']['adsense']['authoradsense']['switch'] == 'enabled' && current_user_can( 'publish_posts' ) ){
    //    add_meta_box(
    //        'posts-meta-authoradsense',
    //        esc_html__( 'Author AdSense' ),
    //        'starcitizenfansitekit_metabox_authoradsense',
    //        'post',
    //        'side',
    //        'default'
    //    );
...
*/
function starcitizenfansitekit_metabox_authoradsense( $object, $box ) { 
    //wp_nonce_field( basename( __FILE__ ), 'postnonce' );
    //$value = get_post_meta( $object->ID, '_starcitizenfansitekit_authoradsense', true );
    //echo '<textarea id="c2pauthoradsense" name="starcitizenfansitekit_post_authoradsense" rows="5" cols="30">'.$value.'</textarea>'; 
} 

/**
* Save meta box's for post posts type
*/
function starcitizenfansitekit_save_meta_boxes_post( $post_id, $post ) {    
    ...
    {  
        ...
        /* Get the meta key. */
        $meta_key = '_starcitizenfansitekit_' . $term;
                                            
        if( isset( $_POST['starcitizenfansitekit_post_'.$term] ) )
        {
            $new_meta_value = $_POST['starcitizenfansitekit_post_'.$term];    
        }
